...
//                D("Ip")->filter();//功能暂时废除
     ...
     *  add_time
     */
    function bindAliNumber($data){
        M('members_ali_number_log')->data($data)->add();
        return M('members_ali_number_log')->getLastSql();
    }
...
//        echo $this->getLastSql();
     ...
     * @return boolean
     */
    function updateSignture($userId, $phone, $signture) {
        $data['signture'] = $signture;
        $data['updateTime'] = time();
        $where['uniqueId'] = $userId;
        $where['phone'] = $phone;
        $id = $this->where($where)->data($data)->save();
     ...
     * @param string $pwd
     */
    function updateRegisterUserInfo($userId, $phone, $pwd, $memberCodeUrl,$cityId,$provinceId,$jpush) {
        $where = array(
            'id' => $userId
        );
...
//            }
    ...
    function saveMemberNews($data) {
        if ($data['uid']) {
            ...
            //echo M('member_verify_info')->getLastSql();die;
        }
        return $res;
    }
     ...
     * @return boolean
     */
    function saveUserInfo($phone, $pwd, $imei, $mobileType = '1', $memberCodeUrl,$cityId,$provinceId,$jpush) {
        $flag = false;
        $loginTime = time();
        $data['imei'] = $imei;
        $data['addTime'] = time();
...
//            }
    ...
    function saveCodeData($phone, $imei, $code, $type = '1') {
        ...
        //var_dump(M('MembersCode')->getLastSql());die;
        return $id;
    }
    ...
    function getUserHome($uid, $friendId) {
        ...
        //$mwhere['freeze'] = '0';
        $fields = 'id AS userId,name AS nickName,image,signture,phone,integral,groupType,memberCodeUrl';
...
//            $resultData['phone'] = substr_replace($memberInfo['phone'], '****', 3, 4);
            ...
            $resultData['shopId'] = M('members_shop')->where("userId = " . $mwhere['id'])->getField('id');
//            var_dump(M()->getlastsql()));exit;
            $resultData['shopName'] = M('members_shop')->where("userId = " . $mwhere['id'])->getField('shopName');
//            $resultData['shopName'] = json_decode($resultData['shopName']);
            $resultData['signture'] = $memberInfo['signture'];
            $resultData['fansNum'] = M('friend')->where("fuid = {$mwhere['id']} and status = '1'")->count();
//            var_dump(M()->getlastsql());exit;
