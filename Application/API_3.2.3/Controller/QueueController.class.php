...
class QueueController extends CommonController {    
    ...
    private $exchangeSwitch = 'exchange_switch_3_2';
    protected $userId;
...
//        var_dump($ACTION_NAME);exit;
        if (in_array($ACTION_NAME, array('switchstatus', 'mianecut'))) {//先判断是否需要验证会员信息
            ...
            if ($phone && $userId) {//判断参数是否为空
                ...
                if ($phone == '12345678900') {
                    $res = $model->field('id,freeze')->where(array('phone' => $phone))->find();
                } else {
                    $t_where['uniqueId'] = $userId;
                    $t_where['phone'] = $phone;
                    $res = $model->field('id,freeze')->where($t_where)->find();
                }
            ...
            foreach($queueList as $k => $v)
            {
                ...
                switch($step)
                {
                    case 10: //根据概率得出是否中奖相关处理
                        ...
                        if($isWinning)
                        {
                            $t_data = array('execStep' => 20);
                            $t_prob = -1;
                        }else
                        {
                            $t_data = array('errorCode' => 11, 'errorMsg' => '概率中奖失败');
                            $t_prob = 1;
                        }
                        $this->execResult($t_where, $t_data, $t_userId, $t_prob);
                        break;
                    case 20: //判断是否还有资金配额可用于此用户的充值面额进行充值
                        $curplus    = $this->getDenominationTotal($v['money']);
                        if($curplus > 0)
                        {
                            $t_data = array('execStep' => 30);
                            $t_prob = -1;
                        }else
                        {
                            $t_data = array('errorCode' => 31, 'errorMsg' => '本时段该面额已用完');
                            $t_prob = 1;
                        }
                        $this->execResult($t_where, $t_data, $t_userId, $t_prob);
                        break;
                    case 30: //用户充值面额所需积分是否充足
                        unset($where);
                        $where = array(
                            'id' => $v['userId']
                        );
                        $integral = M('Members')->field('integral')->where($where)->find();
                        if($integral['integral'] >= $v['integral'])
                        {
                            $t_data = array('execStep' => 40);
                            $t_prob = -1;
                        }else
                        {
                            $t_data = array('errorCode' => 31, 'errorMsg' => '账户积分不足');
                            $t_prob = 1;
                        }
                        $this->execResult($t_where, $t_data, $t_userId, $t_prob);
                        break;
                    case 40: //判断当前用户状态
                        ...
                        $freeze = M('Members')->field('freeze')->where($where)->find();
                        if($freeze['freeze'] == 0)
                        {
                            $t_data = array('execStep' => 50);
                            $t_prob = -1;
                        }else
                        {
                            $t_data = array('errorCode' => 41, 'errorMsg' => '用户状态异常');
                            $t_prob = 1;
                        }
                        $this->execResult($t_where, $t_data, $t_userId, $t_prob);
                        break;
     ...
     * 统一执行 结果
     */
    public function execResult($where, $data, $userId, $prob)
    {
        //修改队列步骤 或 错误码和错误信息
        M('PosterBillQueue')->where($where)->save($data);
        //修改用户概率
        $this->probCumsum($userId, $prob);
    }
    ...
    {
        $where['key'] = $this->exchangeSwitch;
        $rs = M('config_member')->field('value')->where($where)->find();
        $exchange_switch_status = $rs['value'];
    ...
    public function checkExchangeNew()
    {
        ...
        $datas['phone'] = I('post.phone'); //充值的手机号码
        //$data['param']  = json_encode($_POST);
        //$data['cdate']  = date('Ymd', time());
        $datas['time']  = time();
     ...
     * @return array
     */
    public function exchange($userId, $phone, $money) {
        $array  = percent();
        $now = time(); //订单时间
        ...
        //扣除飞币
        $map['id'] = $userId;
        $reuslts = D('Members')->where($map)->setInc('integral', - $array[$money]);
    ...
    function checkUserId($phone, $userId, $fields = "*") {
        ...
        $where['phone'] = $phone;
        $resMember = M('Members')->where($where)->field($fields)->find();
   ...
   function percent() {
       $model = M();
       $arrayData = array();
       $where['key'] = array('like', 'percent_%');
       $data = $model->table("lu_config")->where($where)->select();
       if ($data) {
           foreach ($data as $key => $value) {
               $k = substr($value['key'], 8);
               if (intval($k)) {
                   $arrayData[$k] = $value['value'];
               }
           }
       }
       return $arrayData;
