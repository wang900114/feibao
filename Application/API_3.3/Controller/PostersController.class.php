        ...
        if (is_empty($version) || is_empty($userId) || is_empty($phone)) {//判断参数是否有缺失
            ...
            if ($resCategory) {
                foreach ($resCategory as $k => $v) {//返回最新状态 1：是，2：否
                    ...
                    $resCategory[$k]['paTitle'] = D('Poster')->getHomeCategoryTitle($v['id']);
                    //echo $v['id'];die;
...
//            }
    ...
    public function passVouchers(){
        ...
        if(is_empty($version) || is_empty($userId) || is_empty($phone) ||  is_empty($dataId)){//验证参数
            ...
            $resGetVouchers=M('get_vouchers_log')->field('id,isUse,dataId,type,fuid,userId,vouchersNumber,isWarning,isOfficial')->where($where)->find();
                        ...
                        if($resPoster['userId']==$userId){//判断广告发布人与扫码人是否为同一人
                            ...
                            if($resVouchers){
                                ...
                                if($resPoster['integral']-$resPoster['extendTotalIntegral']>0){    
                                    ...
                                    if($res){
                                        //判断是否增加积分
                                        if($resPoster['integral']-$resPoster['extendTotalIntegral']>0 && ($resGetVouchers['type']==2 && $resGetVouchers['isOfficial']==1) ){//判断广告剩余积分和转发状态
    ...
    public function getvouchers(){
        ...
        $forwardType = I('post.forwardType');
        $officialId = I('post.officialId');
        ...
        if (is_empty($userId) || is_empty($id) || is_empty($phone) ||  is_empty($type)) {//判断参数是否完整
            ...
            if($type==2){//判断转发
                if( is_empty($fuid) || is_empty($forwardId) || is_empty($officialId) || is_empty($forwardType)){
                    $return['status'] = -888;
                    $return['message'] = '传参不完整';
                }
            }
            ...
            $officialId= decodePass($officialId);
            ...
            if (is_bool($res) && empty($res)) {//判断广告状态
                ...
                if($type==2){//判断状态
                    $datas['getStatus']='3';
                    if($forwardType==1){//官方转发
                        //M('OfficialForward')->where('dataId='.$id.' and id='.$officialId)->save($datas);
                        D('Poster')->addOfficialForwadLog($userId,$fuid,$officialId,$datas['getStatus']);
                    }else{//朋友转发
                    ...
                    $where='dataId='.$id;
                    $resVouchers=M('poster_advert_vouchers')->field('id,number,vouchersPrice as give,dataId,content,addTime,status,startTime,endTime,htmlData,shopName,vouchersIntegral')->where($where)->find();
                    //$resVouchers['vouchersIntegral']='每推荐一个好友消费即可获得'.$resVouchers['vouchersIntegral'].'飞币的奖励'.'(可兑换'. $resVouchers['vouchersIntegral']/1000 .'元话费哦)';
                    $resVouchers['vouchersIntegral'] =D('Poster')->getTips($resVouchers['vouchersIntegral']);
                ...
                if($type==1){//直接获取
                    ...
                    if(time()>$res['endTime']){
                        ...
                        if($resVouchers['id']){//查找代金券
                            ...
                            if($forwardType==1){//官方转发
                                //$resForwardNow = M('OfficialForward')->field('id,getStatus')->where('dataId='.$id.' and getStatus!="1"'.' and isUse="1" and isDel="1"')->find();
                                //$resForward = M('OfficialForward')->field('id,getStatus')->where('dataId='.$id.' and id='.$officialId.' and isUse="1" and isDel="1"')->find();
                                $resForwardNow = M('OfficialForwardLog')->field('id,getStatus')->where('friendId='.$userId.' and dataId='.$id.' and getStatus!="1"'.' and isUse="1" and isDel="1"')->find();
                                $resForward = M('OfficialForwardLog')->field('id,getStatus')->where('friendId='.$userId.' and dataId='.$id.' and officialForwardId='.$officialId.' and isUse="1" and isDel="1"')->find();
                               ...
                               // }
                                ...
                                if(time()>$resVouchers['endTime']){
                                    ...
                                    if($forwardType==1){//官方转发
                                         ...
                                         //$resEdit = M('OfficialForward')->where('userId='.  $fuid .' and dataId='.$id.' and id='.$officialId)->save($dataGetThree);
                                         D('Poster')->addOfficialForwadLog($userId,$fuid,$officialId,$dataGetThree['getStatus']);
                                    }else{//朋友转发
                                    ...
                                    if($resadd){
                                        ...
                                        if($forwardType==1){//官方转发
                                            //M('OfficialForward')->where('dataId='.$id.' and id='.$officialId)->save($datas);
                                            D('Poster')->addOfficialForwadLog($userId,$fuid,$officialId,$datas['getStatus'],$datas['getVouchersId']);
                                        }else{//朋友转发
                                            M('FriendForward')->where('friendId='.$userId.' and dataId='.$id.' and id='.$forwardId)->save($datas);
                                        }
                                        ...
                                        $return['status'] = 1;
                                        $return['message'] = '获取成功'. M("OfficialForwardLog")->getLastSql();
                                    }else{
                                        $return['status'] = 10;
                                        $return['message'] = '操作失败';
                                    }
