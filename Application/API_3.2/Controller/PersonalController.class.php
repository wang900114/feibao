...
class PersonalController extends BaseController {
    ...
    public function _initialize() {
        ...
        if (in_array($ACTION_NAME, array('getcode', 'registerphone', 'registerpwd', 'registerinvite', 'registerhandlepassword',
                    'memberlogin', 'checkforgetpasswordcode', 'checkeditforgetpassword', 'getmessage', 'customerservice','test'))) {//先判断是否需要验证会员信息
        } else {
    ...
    public function checkMemberNews() {
        ...
        if (is_empty($handlePassword) || is_empty($version) || is_empty($code) || is_empty($name) || is_empty($identityCard) || is_empty($email) || is_empty($_FILES['imageFirst']['name']) || is_empty($_FILES['imageSecond']['name']) || is_empty($_FILES['imageThree']['name'])) {//判断参数是否为空
            $ret['status'] = 29;
            $ret['message'] = '操作失败';
        } else {
                ...
                if ($resError < 5) {//验证输入错误的次数
                    ...
                    if ($res['handlePassword'] == md5(md5($handlePassword . $res['encrypt']))) {
                        ...
                        if ($resCode) {//判断验证码是否存在
                            ...
                            if(empty($info['imageFirst']['path']) || empty($info['imageSecond']['path']) || empty($info['imageThree']['path'])){//判断图片上传是否成功
                                $ret['status'] = 10;
                                $ret['message'] = '图片超过5M，无法上传';
                                $this->apiCallback($ret);
                            }
    ...
    public function registerPhone() {
        ...
        $phone = I('post.phone');
        $code = I('post.code');
        if (is_empty($phone) || is_empty($code) || is_empty($imei)) {//判断参数是否为空
            $ret['status'] = 29;
            $ret['message'] = '参数不完整';
        } else {
            preg_match('/1[0-9]{10}/', $phone, $matches);
    ...
    public function registerPwd() {
        ...
        $pwd = I('post.password');
        if (is_empty($phone) || is_empty($code) || is_empty($pwd) || is_empty($mobileType) || is_empty($imei)) {//判断参数是否为空
            $ret['status'] = 29;
            $ret['message'] = '参数不完整';
        } else {
            //获取会员信息
                ...
                if ($resCode) {//判断验证码信息是否存在               
                    ...
                    if ($pwdLength == 32) {//判断密码长度
                        ...
                        if ($result) {//判断操作是否成功
                            ...
                            //$nowDay=date("Y-m-d");
                            $nowDay=date("Y-m-d");
                            $file = '/home/wwwroot/api/Uploads/memberCode/'.$nowDay.'/';
    ...
    public function test() {
        ...
        $POST['imei'] = '123456456';
        $POST['userId'] = 54789;
        $POST['phone'] = '15101538925';
        $POST['code'] = '23423';

        $POST['name'] = 'LIFENGHAN';
        $POST['handlePassword'] = '34535345';
        $POST['identityCard'] = '345TETERTE';
        $POST['email'] = 'lifenghan@feibaokeji.com';
        echo $imei = I('post.imei');
        
        $this->checkMemberNews();
        
        
        
        $range = GetDistance('116.493690', '39.922940', '116.495027', '39.923396');
        $return['range'] = judgeDistance($range);
        echo $range . '-' . $return['range'];
        die;

        $model = D('Members');
        $model->clearLoginNum('42900', '13681260087', 5);
        ...
        $result = D('Poster')->addIntegral($userId, $noteId, $id, $forwardUserId, $proRedStart, $proRedEnd, 1);
        var_dump($result);
        die;
    }
    ...
    public function registerInvite() {
        ...
        if (is_empty($imei) || is_empty($phone) || is_empty($userId) || is_empty($inviteCode)) {//判断参数是否为空
            $ret['status'] = 29;
            $ret['message'] = '参数不完整';
        } else {
            $model = D("Members");
        ...
        $this->apiCallback($this->ret);exit(0);
    }
    ...
    public function registerHandlePassword() {
        ...
        if (is_empty($imei) || is_empty($phone) || is_empty($userId) || is_empty($handlePassword)) {//判断参数是否为空
            $ret['status'] = 29;
            $ret['message'] = '参数不完整';
        } else {
            $model = D("Members");
    ...
    public function memberLogin() {
        ...
        if (is_empty($phone) || is_empty($pwd) || is_empty($imei)) {//验证参数是否为空
            $ret['status'] = 29;
            $ret['message'] = '参数不完整';
        } else {
            //验证手机号码格式、长度
                        ...
                        if ($flag) {//判断密码是否正确
                            ...
                            if ($type == 1) {//验证修改登录密码
                                ...
                                if ($resError >= 3) {
                                    ...
                                    $ret['status'] = 23;
                                    //$ret['message'] = '密码输入错误!'.(3 - $resError);
                                    $ret['message'] = '操作码错误，请重新输入(' . (3 - $resError) . ')';
                                    $ret['getTimes'] = 3 - $resError;
                                }
                            }
                        }
    ...
    public function editPassword() {
        ...
        $type = I('post.type');
        $imei = I('post.imei');
        if (is_empty($userId) || is_empty($pwd) || is_empty($type) || is_empty($newPwd) || is_empty($phone) || is_empty($imei)) {//判断参数是否为空
            $ret['status'] = 29;
            $ret['message'] = '参数不完整';
        } else {
            if (($type == 1) || ($type == 2)) {//判断type值是否正确
    ...
    public function checkHandlePassword() {
        ...
        if (is_empty($userId) || is_empty($handlePassword) || is_empty($phone) || is_empty($imei)) {//判断参数是否为空
            $ret['status'] = 29;
            $ret['message'] = '参数不完整';
        } else {
            //验证唯一码是否正确
    ...
    public function detail() {
        ...
        if (is_empty($userId) || is_empty($phone)) {//判断参数是否为空
            ...
            $model = D("Members");
            // 验证userId
            $field = 'id,uniqueId as userId,name,jpush,phone,image,imei,imageUrl,integral,invite,cityId,provinceId,freeze,handlePassword,type,groupType,memberCodeUrl';
    ...
    function checkOldPhone() {
        ...
        if (is_empty($userId) || is_empty($phone) || is_empty($code) || is_empty($handlepwd)) {
            ...
            if ($userIdStatus && $codeStatus) {//当以上两个验证都成功
                $handlepwdStatus = $model->checkHandlePwd($userId, $phone, $handlepwd); //验证操作码是否正确
                if ($handlepwdStatus) {
                    $this->ret['status'] = 1;
                    $this->ret['message'] = '操作成功';
                }
