...
class PosterModel extends CommonModel {
     ...
     * $userId 获取人会员id
     * $friend 官方推荐会员id
     * $officialId 官方推荐的记录id
     * $getStatus 更新的状态
     * $getVouchersId 
     */
    public function addOfficialForwadLog($userId,$friend,$officialId,$getStatus,$getVouchersId){
        $res = M('OfficialForward')->where('id='.$officialId)->find();
        if($res['id']){
            $res['userId'] = $friend;
            $res['friendId'] = $userId;
            $res['getStatus'] = $getStatus;
            $res['officialForwardId'] = $officialId;
            unset($res[id]);
            //var_dump($res);die;
            if($getVouchersId){
                $res['getVouchersId'] = $getVouchersId;
            }
            
            $result = M("OfficialForwardLog")->data($res)->add();
            //echo M("OfficialForwardLog")->getLastSql();
     ...
     * 获取代金券后给出的提示语
     */
    public function getTips($integral){
        //'每推荐一个好友消费即可获得'.$resVouchers['vouchersIntegral'].'飞币的奖励'.'(可兑换'. $resVouchers['vouchersIntegral']/1000 .'元话费哦)';
        ...
        if($integral){
            $integral =$integral * $messageInfo['value'];
            $res = $resVouchers['vouchersIntegral']='每推荐一个好友消费即可获得'.$integral.'飞币的奖励';
            
            $res .='（可兑换';
            $configFC=M('ConfigFc')->field('value')->where('`key` ="percent"')->find();
            //if(empty($configFC['value'])){
                //$configFC['value']=2000;
            //}
            $money = ($integral/$configFC['value']);
            //echo $money;die;
            if($money<1){
                $res .=$money * 100 .'分';
            }else{
                ...
                //大于1元，显示元分
                //$resTmp =  explode('.',$money);
                
                //if($resTmp[0]){
                    //$res .=$resTmp[0] .'元';
                //}
                
                //echo $resTmp[0].'-'.$resTmp[1].'-'.$resTmp[1]/10;
...
//        $map['nickName'] = array('!=', 'app下载');
    ...
    //获取分类下的广告信息
    public function getHomeCategoryTitle($typeId){
        if($typeId){
            $field = 'pact.id,pact.dataId,pa.title';
            $where = ' pact.typeId='.$typeId;
            $nowTime = time();
            $where .= ' and pact.setTime>'.$nowTime;
            $where.=' and pa.startTime <' . $nowTime; //是否在开始时间之后
            $where.=' and pa.endTime >' . $nowTime; //是否在结束时间内
            $where.=' and pa.status ="1"'; //广告状态是否正常
            $where.=' and pa.integral >pa.exposeTotalIntegral + pa.extendTotalIntegral';
            $where.=' and pact.status ="1"';
            
            $order = 'id desc';
            $join = 'AS pact LEFT JOIN lu_poster_advert AS pa ON pact.dataId = pa.id';
            $res = M('PosterAdvertCategoryTitleManageLog')->field($field)->where($where)->order($order)->join($join)->select();
            //echo M('PosterAdvertCategoryTitleManageLog')->getLastSql();die;
            if($res){
                $count = count($res);
                $tmpRand = mt_rand(0,$count-1);
                //echo $res[$tmpRand]['title']; die;
                return $res[$tmpRand]['title'];
            }
        }
     ...
     * 根据编号获取代金券id
     */
    public function getVouchersId($vouchersNum){
        $where = 'vouchersNumber="'.$vouchersNum.'"';
        //$re = M('poster_advert_vouchers')->field('id')->where($where)->find();
        $re = M('get_vouchers_log')->field('id')->where($where)->find();
        if (empty($re))
            $re = array();
     ...
     * 删除我的扫码记录
     */
    public function delMySanCode($userId){
        if($userId){
            $where = 'flag="1"';
            $where.=' and userId='.$userId;
            $data['flag']='2';
            $res = M('ScanVouchersLog')->where($where)->save($data);

            if($res){//添加清空日志
                $datas['userId'] = $userId;
                $datas['addTime'] = time();

                M('members_delall_scan_vouchers_log')->data($datas)->add();
            }
            return $res;
        }else{
            return 0;
     ...
     * 获取我的扫码记录
     */
    public function getMyScanCodeList($userId,$selectTime,$type,$page,$pageSize){
        $selectTime = $selectTime <= 0 ? time() : $selectTime;
        ...
        $where = 'flag="1"';
        $where.= ' and userId='.$userId;
        $field='id,userId,type,flag,vouchersNum,dataId,addTime';
        
        if($type){
            $where.=' and addTime<'.$selectTime;
            
        }
        
        $re = M('ScanVouchersLog')->field($field)->where($where)->order('addTime desc')->limit($limit)->select();
        //echo M('ScanVouchersLog')->getLastSql();die;
     ...
     * @param int $pageSize 请求的页数
     * @param int $pageSize 每页显示的条数
     * @param int $type 状态 1：加载，2：刷新
     * @param int $categoryType 当前状态 1：当前，2：历史
     * @param int $selectTime  时间 
     */
    public function getMyVouchersList($userId,$page,$pageSize,$type,$categoryType,$selectTime){
        $selectTime = $selectTime <= 0 ? time() : $selectTime;
        $page = empty($page) ? 1 : $page;
        $limit = ($page - 1) * $pageSize . "," . $pageSize;
        
        $field = 'pav.number,pav.vouchersPrice as give,pav.dataId,pav.content,pav.shopName,pav.status,pav.startTime,pav.endTime,pav.htmlData,gv.vouchersCodeUrl,gv.id,gv.type,gv.isUse,gv.addTime,gv.vouchersId,gv.vouchersNumber';
        $join = " AS gv LEFT JOIN lu_poster_advert_vouchers AS pav ON gv.vouchersId = pav.id";
        
        $where ='gv.userId =' . $userId;
        $where.=' and gv.isDraw ="1"';
        $where.=' and gv.isDel ="1"';
        
        if($selectTime){//判断添加时间
            $where.=' and gv.addTime <'.$selectTime;
        }
        
        $nowTime=time();
        //echo $categoryType.'-';
        if($categoryType==1){//当前未使用或者未开始
            //$where.=' and pav.endTime>'.$nowTime;
            //$where.=' and ( gv.isUse="1" or pav.startTime>'.$nowTime.')';
            $where.=' and  gv.isUse="1" and pav.endTime>'.time();
        }else{
            $where.=' and ( pav.endTime<='.$nowTime .' or gv.isUse="2" )';
        }
        
        $re = M('get_vouchers_log')->field($field)->where($where)->order('addTime desc')->join($join)->limit($limit)->select();
        ...
        if (empty($re)){
            ...
            foreach ($re as $key => $value) {
                ...
                //voucherStatus:状态 1-未开始，2-未使用，3-已作废，4-已使用
                if($categoryType==1){
                    if($value['startTime']>$nowTime){
                        $re[$key]['voucherStatus']=1;
                    }else{
                        $re[$key]['voucherStatus']=2;
                    }
                }else{
                    if($value['isUse']==2){
                        $re[$key]['voucherStatus']=4;
                    }else{
                        $re[$key]['voucherStatus']=3;
                    }
                }
                
                $re[$key]['number']= $value['vouchersNumber'];
                $resVouchers = M('PosterAdvertVouchers')->field('vouchersPrice,vouchersIntegral')->where('dataId ='.$value['dataId'])->find();
                if($resVouchers){
                    $re[$key]['vouchersIntegralMessage'] =$this->getTips($resVouchers['vouchersIntegral']);
                }
                
                //$re[$key]['vouchersId']= encodePass($re[$key]['vouchersId']);
                $re[$key]['vouchersId']= encodePass($value['id']);
                $re[$key]['dataId']= encodePass($value['dataId']);
                $re[$key]['id']= encodePass($value['id']);
                if($re[$key]['vouchersCodeUrl']){
                    $re[$key]['vouchersCodeUrl']=WEBURL .'/'.$value['vouchersCodeUrl'];
                }
...
//        }
    ...
    public function getPosterAdvert($dataId, $field = 'id,title,lngMin,imageUrl,lngMax,latMax,pushCityId,latMin,address,type,status,pushType,warnPhone,shareUrl,collectTotal,weburl,addTime,startTime,endTime,integral,exposeTotalIntegral,extendTotalIntegral') {
        ...
        if ($re) {//获取标签
            ...
            }elseif($re['type'] == 2){//取优惠信息
                $htmlData = M("poster_discount")->where($where)->field($fields)->find();
            }elseif($re['type'] == 3){//取公益广告信息
                //$htmlData = array();
                $htmlData = M("PosterCommonweal")->where($where)->field($fields)->find();
            }elseif($re['type'] == 4){//取代金券信息
                $htmlData = M("poster_advert_vouchers")->where($where)->field($fields)->find();
            }else{
                $htmlData = array();
            }
            ...
            $re['give'] = $htmlData['give'];
            $re['content'] = $htmlData['content'];
            $re['number'] = $htmlData['number'];
            $re['voucherStartTime'] = $htmlData['startTime'];
            $re['voucherendTime'] = $htmlData['endTime'];
            $re['shopName'] = $htmlData['shopName'];
            //$re['goodsImage'] = $htmlData['goodsImage'];
            //$re['goodsLink'] = $htmlData['goodsLink'];
     ...
     *获取我的所有朋友 
     */
    public function getMyAllFriendsListData($userId,$type=1){
        //$limit = '500';
        ...
        $res = M('Friend')->field($field)->where($where)->order($order)->join($join)->select();
    ...
    public function addClickTotal($id, $type = 1,$userId,$id) {
        if ($type == 1) {
            $data['dataId']=$id;
            $data['userId']=$userId;
            $data['addTime']=time();
            
            M('ClickPosterLog')->data($data)->add();
