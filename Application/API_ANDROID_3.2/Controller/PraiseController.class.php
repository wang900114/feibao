...
class PraiseController extends CommonController {
    ...
    private function categoryId2ModelName($type) {
        switch ($type) {  // 1头条；2店铺；3海报；4发现；5头条轮播图；6本地新闻
            case '6':
                $modelName = 'PraiseLocalnewsLog';
                break;
            case '5':
                $modelName = 'PraiseNewsCarouselLog';
                break;
            case '4':
                $modelName = 'PraiseFoundLog';
                break;
            case '3':
                $modelName = 'PraisePosterLog';
                break;
            case '2':
                $modelName = 'PraiseShopLog';
                break;
            case '1':
                $modelName = 'PraiseNewsNormalLog';
                break;
        }
        return $modelName;
    }
            ...
            if (empty($data)) {
                ...
                if (is_bool($exist) || !empty($exist)) {
                    ...
                    if ($res == true) {
                        if ($type == 4) {
                            //更新发现表 赞数量
                            D('found')->setColunm(array('id' => $dataId), 'praiseNum', 1);
                        }
                        $return['status'] = 1;
                        $return['success'] = true;
                        $return['message'] = '添加成功';
