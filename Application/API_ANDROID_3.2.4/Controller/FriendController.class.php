...
class FriendController extends BaseController {
    ...
    function forward() {
        ...
        if (is_empty($userId) || is_empty($pageSize) || is_empty($page)) {
            $return['status'] = 10;
            $return['message'] = '操作失败';
        } else {
            $reLists = D("Friend")->forwardNew($userId, $lastTime, $page, $pageSize, $selectTime);
            //var_dump($reLists);die;
            if($page==1){
                //获取官方转发的信息
                $reOfficialList = D("Friend")->officialForward($userId);
    ...
    function searchForward() {
        ...
        if (is_empty($userId) || is_empty($pageSize) || is_empty($page) || is_empty($selectTime) || is_empty($lastTime) || is_empty($content)) {
            ...
            $reList = D("Friend")->searchForwardNew($userId, $content, $lastTime, $page, $pageSize, $selectTime);
            if (empty($reList)) {
                $return['status'] = 36;
                $return['message'] = '查询成功，暂无数据';
                $return['info'] = array();
    ...
    /**
     * 3.2搜索朋友转发给我的广告列表-本次更新后删除
     */
    function searchForwardOld() {
        $return['success'] = true;
        $pageSize = I('post.pageSize', 10);
        $page = I('post.page', 1);
        $selectTime = I("post.selectTime", time());
    ...
    function personalForward() {
        ...
        if (is_empty($userId) || is_empty($pageSize) || is_empty($page) || is_empty($selectTime) || is_empty($friendId) ) {
            ...
            //$forwardType 1-官方转发，2-朋友转发
            if($forwardType==1){
                $reList = D("Friend")->personalOfficialForward($friendId, $userId, $page, $pageSize, $selectTime);
            }else{
                $reList = D("Friend")->personalForwardNew($friendId, $userId, $page, $pageSize, $selectTime);
            }
            ...
            if (empty($reList)) {
                ...
                if($forwardType==1){
                      foreach ($reList as $k => $v) {
                        ...
                        $resUserFriend= D('Members')->getUserInfo($friendId);

                        if ($resUserFriend['id']) {
                            $reList[$k]['forWardsUserId'] = encodePass($friendId);
                            $reList[$k]['forWardsNickname'] = $resUserFriend['name'];
                            $reList[$k]['forWardsUserImage'] = $resUserFriend['image'];
                        }else{
                            $reList[$k]['forWardsUserId'] = '';
                            $reList[$k]['forWardsNickname'] = '';
                            $reList[$k]['forWardsUserImage'] ='';
                        }

                        $reList[$k]['fuid'] =  encodePass($friendId);

                        $reList[$k]['vouchersIntegral']='每推荐一个好友消费即可获得'.$value['vouchersIntegral'].'飞币的奖励';
                        ...
                        $reList[$k]['forwardUserId'] = encodePass($v['uid']);
                        if ($resUser['id']) {
                            $reList[$k]['userId'] = encodePass($v['userId']);
                            $reList[$k]['nickname'] = $resUser['name'];
                            $reList[$k]['userImage'] = $resUser['image'];
                        }
                    }   
                }else{
                    foreach ($reList as $k => $v) {
                        ...
                        //判断转发人或者朋友
                        if( $v['isForward']==1){//1-转出，2-转来
                            //获取转发人信息
                            $resUserForward = D('Members')->getUserInfo($userId);
                            if ($resUserForward['id']) {
                                $reList[$k]['forWardsUserId'] = encodePass($userId);
                                $reList[$k]['forWardsNickname'] = $resUserForward['name'];
                                $reList[$k]['forWardsUserImage'] = $resUserForward['image'];
                            }else{
                                $reList[$k]['forWardsUserId'] = '';
                                $reList[$k]['forWardsNickname'] = '';
                                $reList[$k]['forWardsUserImage'] ='';
                            ...
                            $reList[$k]['fuid']= encodePass($userId);
                            //$reList[$k]['fuid']= $userId;
                        }else{
                            //获取朋友信息
                            $resUserFriend= D('Members')->getUserInfo($friendId);
