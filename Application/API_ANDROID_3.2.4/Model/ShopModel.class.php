...
class ShopModel extends CommonModel {
	...
	function getCommonStores(){
		...
		if(empty($type)){
			...
			if(empty($reTmp)){
				...
				// $map['id'] = array('neq',$id);//因为是根据ID截取，故而不能排除id
	...
	function getSearchStores(){
		...
		$map['name|tag1|tag2|tag3|tag4'] = array('like','%'.$search.'%');
		$map['cityId'] = $cityId;
	...
	public function getListData(){
		...
		if(empty($type)){//刷新
			...
			if( $otherLimit > 0 ){//数量不够
				...
				return empty($re) ? $re2 : array_merge($re,$re2);//最后的结果合计是 id倒序
			}
	...
	public function isUniqueMobile($value='') {
		$storeId = base64_decode(I('post.storeId'));//解密
		$where = array(
