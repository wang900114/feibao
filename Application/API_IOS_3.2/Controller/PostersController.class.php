...
//            }
    ...
    public function friendPosterDetail() {
        ...
        if (is_empty($userId) || is_empty($id)) {//判断参数是否完整
            ...
            $field = 'id,title,address,type,status,integral,warnPhone,userId,collectTotal,lngMax,shareUrl,latMax,lngMin,latMin,pushCityId,pushType,exposeTotalIntegral,extendTotalIntegral,weburl,addTime,proRedStart,proRedEnd,type,startTime,endTime';
            $res = D('Poster')->getPosterAdvert($id, $field); //查询广告基本信息
            if (is_bool($res) && empty($res)) {//判断广告状态
                $return['status'] = -1;
                $return['message'] = '查询失败';
